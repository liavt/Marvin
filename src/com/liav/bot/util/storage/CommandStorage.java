...
public final class CommandStorage {
	...
	public final static Command[] commands = { new Command("newfag",
			...
			new Command("hack", "Usage: hack [player]\nHax0rs someone", "fun", (String[] param) -> {
				...
				sb.append("Succesfully hacked ").append(u.getName()).append(". Results are as followed: ").append("\n");
				sb.append(AutomodUtil.timeToString((int) Long.parseLong(id.substring(0, 7)))).append(" of porn")
						.append("\n");
				sb.append(AutomodUtil.timeToString((int) Long.parseLong(id.substring(8, 15)))).append(" of dank memes,")
						.append("\n");
			...
			new Command("play",
...

				{
							...
							// works for now.
							final File directory = new File("./lib/output/"),
									out = new File(directory.getAbsolutePath() + "/out.mp3");
							File preconversion = new File(directory.getAbsolutePath() + "/out.mp3");
							if (directory.exists()) {
							...
							final ProcessBuilder p = new ProcessBuilder(new String[] { "lib/youtube-dl.exe", "-f",
									"bestaudio[filesize<10m][protocol=https]", "--extract-audio", "-o",
									preconversion.getAbsolutePath(), param[0] }).inheritIO();
							int code = p.start().waitFor();
							if (code < 0) {
								return "Error retrieving video! Are you sure the URL is correct?";
							}
			...
			new Command("fate", "Usage: fate [*optional* player]\nTells the fate of a person.", "fun",
					(final String[] p, final IUser user) -> {
						...
						return fates[Bot.random.nextInt(fates.length - 1)].replace("%U%", u.getName());
					}) };
}
